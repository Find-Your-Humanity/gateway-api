-- 단계별 API 키 스키마 업데이트
-- 각 단계를 개별적으로 실행하여 오류를 방지

-- ============================================
-- 1단계: api_keys 테이블 생성 (이미 존재하면 무시)
-- ============================================
...
-- ============================================
-- 2단계: allowed_origins 컬럼 추가
-- ============================================
-- 이 단계를 실행하기 전에 api_keys 테이블이 존재하는지 확인하세요
-- 만약 컬럼이 이미 존재한다면 오류가 발생할 수 있습니다

-- 방법 1: 직접 추가 (컬럼이 없을 때만)
-- ALTER TABLE api_keys ADD COLUMN allowed_origins TEXT COMMENT '허용된 도메인 목록 (JSON 배열)';

-- 방법 2: 안전한 추가 (동적 SQL 사용)
SET @sql = IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'api_keys' 
     AND COLUMN_NAME = 'allowed_origins') = 0,
    'ALTER TABLE api_keys ADD COLUMN allowed_origins TEXT COMMENT ''허용된 도메인 목록 (JSON 배열)''',
...
WHERE TABLE_SCHEMA = DATABASE() 
AND TABLE_NAME IN ('api_keys', 'captcha_tokens')
